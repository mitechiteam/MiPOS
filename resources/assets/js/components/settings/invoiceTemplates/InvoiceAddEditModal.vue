                        ...
                        <div class="form-row">
                            <div class="form-group col-6">
                                ...
                                <select v-validate="'required'" data-vv-as="type" name="template_type"
                                        v-model="template_type" v-on:change="changeType()" id="templateType" class="custom-select">
    ...
    export default {
        ...
        data() {
            return {
                ...
                content: '',
                isCustom: '',
                invoiceKey: ["{app_logo}", "{app_name}", "{table_name}", "{invoice_id}",
                    "{supplier_name}", "{employee_name}", "{date}", "{time}", "{item_details}",
                    "{sub_total}", "{total}", "{payment_details}", "{exchange}", "{due}",
                    "{customer_name}", "{phone_number}", "{address}", "{barcode}", "{tin}"],
                is_disabled: false,
        ...
        methods: {
            ...
            changeType()
            {
                if (this.template_type == "receiving") {
                    const shipmentInfoIndex = this.invoiceKey.indexOf('{shipment_info}');
                    if (shipmentInfoIndex >= 0) {
                        this.invoiceKey.splice(shipmentInfoIndex,1);
                    }
                    const noteIndex = this.invoiceKey.indexOf('{note}');
                    if (noteIndex >= 0) {
                        this.invoiceKey.splice(noteIndex,1);
                    }

                } else {
                    this.invoiceKey = [...this.invoiceKey,"{shipment_info}", "{note}"]
                }
            },
            save() {
            ...
            getInvoiceTemplateData(route) {
                ...
                instance.axiosGet(route,
                    function (response) {
                        ...
                        //showing shipment ifo only for sales invoice
                        if (instance.template_type == "sales") {
                            instance.invoiceKey.push('{shipment_info}', '{note}');
                        }
