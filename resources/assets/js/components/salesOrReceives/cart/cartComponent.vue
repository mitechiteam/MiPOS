    ...
    <span>
        <div id="cart-section-1" class="modal-layout-header p-0">
            <div v-if="isSelectedBranch && customerNotAdded" class="sales-search p-2">
                <div class="row">
                    <div :class="{'col-10': addCustomer=='manage', 'col-12':(addCustomer!='manage' || salesOrReceivingType == 'internal' || salesOrReceivingType == 'internal-transfer'), 'pr-0':(order_type == 'sales' && salesOrReceivingType == 'customer' && addCustomer == 'manage')}">
                                        ...
                                        <a href="#"
                                           ...
                                           @click.prevent="selectCustomer(customer)">
                                            <h6 class="m-0">
                                                ...
                                                <small v-show="customer.email != null"><i class="la la-envelope"/> {{customer.email}}</small>
                ...
                <h6 class="m-0 cart-product-details-parent">
                    <div class="cart-product-details-child">
                        ...
                        <br>
                         ...
                         <small v-show="customer.phone_number != null"> <i class="la la-phone-square"></i> {{customer.phone_number}}</small>
                    </div>
        ...
        <div id="cart-section-3" class="position-absolute fixed-bottom product-card-font">
            ...
            <div class="row mx-0 px-3 py-2  border-bottom" id="pop_mouse1" v-popover:foo.right
                ...
                <div class="col-3 p-0 text-right">
                    ...
                    <!--data-toggle="popover"-->
                    <span v-else>
                        <i class="la la-edit myicon1"></i>{{ numberFormat(discount)+'%' }}
                    </span>
                </div>
            </div>
            ...
            <div class="row mx-0 px-3 py-2 border-bottom">
                <div class="col-6 p-0">
                    ...
                    <span v-if="currentBranch !== null && salesOrReturnType!=='returns' && order_type=='sales' && this.sales_or_receiving_type != 'internal-transfer'">
                        <span class="">( {{trans('lang.tax')}}</span>
                        <span v-if="user.tax_excluded == '0'">{{trans('lang.included')}})</span>
                ...
                <div class="col-3 p-0 text-right">
                    <span v-if="currentBranch !== null && salesOrReturnType!=='returns' && order_type=='sales' && this.sales_or_receiving_type != 'internal-transfer'">
                        ...
                        ><i class="la la-edit myicon1"></i></a>
                    </span>
            ...
            <div class="p-3 border-bottom"
                ...
                <button class="btn pay-btn app-color"
                        ...
                        :disabled="enableDisablePay() || disabledPay() || !isConnected && offline == 0 || isHoldOrderDone === true">
                    {{trans('lang.place_order')}}
                </button>
            </div>

            <!-- Pay button for restaurant -->
            <div v-if="currentBranch !== null && currentBranch.branch_type === 'restaurant' && salesOrReceivingType === 'customer' && !isPlaceOrderActive"
           ...
           <div v-if="(currentBranch !== null && currentBranch.branch_type !== 'restaurant' && salesOrReceivingType !== 'internal' && salesOrReceivingType !== 'internal-transfer') || (currentBranch !== null && currentBranch.branch_type === 'restaurant' && salesOrReceivingType != 'internal'  && (order_type === 'receiving' || salesOrReturnType =='returns'))"
                class="p-3 border-bottom">
               <button class="btn pay-btn app-color"
                       data-toggle="modal"
                       data-target="#cart-payment-modal"
            ...
            <div v-if="order_type=='sales'" class="row mx-0">
                ...
                <a v-else-if="salesOrReceivingType== 'customer'"
                   href="#"
                   class="col-4 p-0 text-center border-right hold-items"
                   :class="{'disabled':customerHoldOrders.length == 0 ||cart.length != 0 || salesOrReturnType == 'returns' }"
                   @click.prevent="openHoldOrderModal">
                    <i class="la la-recycle la-2x p-2 app-color-text hold-icon"></i>
                    <span class="badge badge-danger hold-items-badge-position"
                          v-if="countHoldOrder>0">{{countHoldOrder}}</span>
                </a>
                <!--   <a v-else href="#" data-toggle="modal" data-target="#hold-orders-modal"
                       class="col-4 p-0 text-center border-right hold-items"
...
<script>
    ...
    export default {
        ...
        created() {
            if (this.user.tax_excluded == '0') {
                this.isTaxExcluded = false;
            } else {
                this.isTaxExcluded = true;
            }
        ...
        methods: {
            checkDiscountType() {
                if (this.order_type == 'sales') {
                    if (this.sales_or_receiving_type == 'internal-transfer') {
                        return false;
                    }
                    return true;
                }
            ...
            setQuantityInCart(value, index) {
                let instance = this;
                value = Number(value);
                if (this.outOfStockProduct == 1 && value > this.cart[index].availbleQuantity && this.order_type == 'sales') {
            ...
            commonMethodForAccessingShortcut(data) {
                if (data == 'addCustomerShortcut' && this.shortcutKeyInfo.addCustomerShortcut.status == 1 && this.shortcutStatus == 1) {
                    $('#customer-add-edit-modal').modal('show');
