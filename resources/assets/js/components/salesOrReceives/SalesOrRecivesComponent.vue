<template>
    <div class="main-layout-wrapper" v-if="manage_sales == 1 || manage_receives == 1">
            ...
            <div style="flex: 1 0;">
                                ...
                                <div class="dropdown keep-inside-clicks-open w-100 product-category-filter">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                                ...
                                                <template v-else>
                                                    <div class="col-6" v-for="category in categories">
                                                        <div class="custom-control custom-checkbox">
                                                            <input type="checkbox" class="custom-control-input" :id="['product-category']+category.id" :value="category.id" v-model="categorySearchValue">
                                                            <label class="custom-control-label" :for="['product-category']+category.id">{{ category.name }}</label>
                                                        </div>
                                                    </div>
                                                </template>
                                            </div>
                                        ...
                                        <div class="d-flex justify-content-between align-items-center filter-footer">
                                            <a href="#" class="app-color-text" @click.prevent="clearCategorySearch">{{ trans('lang.clear') }}</a>
                                            <button class="btn app-color" @click.prevent="categorySearch">{{ trans('lang.apply') }}</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div v-else>
                    <div class="row all-products mr-0 custom-scrollbar">
                        <div v-if="salesOrReturnType == 'sales' || order_type =='receiving' && receiveOrReturnType != 'returns'"
                    ...
                    <div v-else>
                        ...
                        <div class="list-group" v-if="order_type=='sales'">
                            ...
                            <a href="#" class="list-group-item list-group-item-action"
                               ...
                               @click.prevent="selectSalesOrReturnType('shipment_list')">{{trans('lang.shipment_list')}}</a>
                        </div>
        ...
        <div class="main-layout-card">
            <sales-list-component v-if="salesOrReturnType=='Sales List' && isSalesListComponentActive"
                                  :branch_id="selectedBranchID"
                                  @resetBranchAndCashRegisterModal="resetBranchAndCashRegisterModal"/>

            <shipment-list-component v-if="salesOrReturnType=='shipment_list' && isShipmentListComponentActive"
                                     :branch_id="selectedBranchID"
            />
        </div>
        <!--End due payment component-->
                    ...
                    <div>
                        ...
                        <div class="list-group">
                            ...
                            <a href="#" class="list-group-item list-group-item-action" v-if="order_type=='sales'"
                               ...
                               @click.prevent="selectSalesOrReceivingType('internal-transfer')">{{trans('lang.internal_transfer')}}</a>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    import {productConverter, concatProductArray} from './helper/productConverter'
    ...
    export default {
        ...
        watch: {
            ...
            isConnected: function (value) {
                ...
                if (this.offline == 1 && this.order_type == 'sales') {
                    if (value) {
        ...
        created() {
            ...
            if (this.user.tax_excluded == "included") {
                this.isTaxExcludedFromCart = false;
            } else this.isTaxExcludedFromCart = true;
            ...
            if (screen.width > 667) {
                ...
                if (this.sales_return_status == 'Sales List' || this.sales_return_status == 'shipment_list') {
                    this.isCartComponentActive = false;
                } else this.isCartComponentActive = true;
            } else {
                this.isCartComponentActive = false;
                this.toggleCart = true;
            }

            if (this.sales_return_status == "Sales List") {
                this.isSalesListComponentActive = true;
            } else this.isSalesListComponentActive = false;

            if (this.sales_return_status == "shipment_list") {
                this.isShipmentListComponentActive = true;
            } else this.isShipmentListComponentActive = false;
        ...
        mounted() {
            ...
            if (this.salesOrReturnType == "shipment_list" || this.salesOrReturnType == "Sales List") {
                // if (this.salesOrReturnType != 'Sales List' || this.salesOrReturnType != 'shipment_list') {
            } else {
            ...
            $('#tax-edit-modal').on('hidden.bs.modal', function (e) {
        ...
        methods: {
            ...
            selectSalesOrReturnType(value) {
                ...
                if (value == 'Sales List' || value == 'shipment_list') {
                    instance.isCartComponentActive = false;
                ...
                instance.axiosGETorPOST(
                    ...
                    (success, responseData) => {
                        ...
                        if (success) //response after then function
                        {
                            ...
                            if (instance.salesOrReturnType == "Sales List") {
                                this.isSalesListComponentActive = true;
                            } else this.isSalesListComponentActive = false;

                            if (instance.salesOrReturnType == "shipment_list") {
                                this.isShipmentListComponentActive = true;
                            } else this.isShipmentListComponentActive = false;
                ...
                this.axiosGETorPOST(
                    {
                        ...
                        postData: {orderID: this.orderID, orderIdInternalTransfer: this.orderIdInternalTransfer} //set post data
                    },
            ...
            setCartItemsToCookieOrDB(flag = 0) {
                let obj = {
                    ...
                    // cookieData
                    cookieData = cartItemsToCookie(flag, obj, this.appVersion);

                this.setCookieDataToGlobal(cookieData);
                //subTotalAmount

                let subTotalAmountMethodData = subTotalAmount(this.salesOrReturnType,this.receiveOrReturnType, this.total, this.profit, this.tax, this.subTotal, this.cart, this.productTotalWithoutDiscount, this.isTaxExcludedFromCart);
                            ...
                            if (instance.currentBranch) //check if branch is selected by user previously
                            {
                                instance.branchList.forEach(function (branch) {
                                    if (instance.currentBranch.id == branch.id) //checks if branch id matches
                                    {
                                        ...
                                        instance.isShipmentEnable = branch.is_shipment; //Shipment Enable or not isShipmentEnable
            ...
            setHoldOrderToCart(holdItem) {
                ...
                if (instance.cart.length == 0) {
                    ...
                    if (instance.salesOrReceivingType == 'internal') {
                        ...
                        instance.internalTransferHoldOrders = instance.setHoldToPending(instance.internalTransferHoldOrders, holdItem.orderID, holdItem.invoice_id, 'internal-transfer');
                    } else {
            ...
            selectOrder(order) {
                ...
                if (instance.cart.length == 0) {
                    ...
                    let value = _.filter(order.cart, function(product) {
            ...
            setTaxIncludedOrExcludedFromCart(value) {
                ...
                // setTaxExcludedIncludedCookie(value);
                this.setCartItemsToCookieOrDB();
            },
