...
//        return ['products' => []];
    ...
    public function salesStore(Request $request)
    {
        ...
        if (($orderStatus == 'done' && !$orderID) || ($orderStatus == 'pending' && !$orderID) || ($orderStatus == 'hold' && !$orderID)) {
            ...
            if ($orders['total'] < 0) {
                ...
                if ($totalProductInThisInvoice - $totalReturnedProduct == $totalCartProduct) {
                    Order::updateOrderType($carts[0]['invoiceReturnId'], 'fully', $orderType);
                } else {
                    Order::updateOrderType($carts[0]['invoiceReturnId'], 'partial', $orderType);
                }
                $orders['order_type'] = $orderType;
            }
        ...
        if (($orderStatus == 'done' && $orderID == null)) {
            ...
            if ($orderStatus == 'done') {
                ...
                try {
                    ...
                    $autoSmsRecive = Setting::getSettingValue('sms_recive_to_customer')->setting_value;


                    if ($orderDetails->customer_id) {
                        $orderCustomer = Customer::getOne($orderDetails->customer_id);

                        if ($autoSmsRecive == 1 && $orderCustomer->phone_number) {
    ...
    public function storeShippingInformation($request, $orderID)
    {
        ...
        $shippingLastId = ShippingInformation::store($ShippingData);
    }
