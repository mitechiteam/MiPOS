...
//        $count = ProductVariant::alreadyExisted($productData['sku'], $productData['bar_code'], $productData['title'], $productData['product_type']);
...
//            DB::rollback();
    ...
    public function adjustStockData(Request $request)
    {
        ...
        foreach ($data as $item) {
            ...
            $productVarientId = $item['product'];            
            $varientProduct = Product::allDetailsByVarientId($productVarientId);

            $itemSubTotal = ($varientProduct->price * $item['stockQuantity']);
            $subTotalAmount += $itemSubTotal;
            if ($varientProduct->tax_percentage != null && isset($itemSubTotal)) {
                $totalTax = ($itemSubTotal * $varientProduct->tax_percentage) / 100;
            }
        }
        $orderData = [
            'date' => date('Y-m-d'),
        ...
        foreach ($data as $item) {
            $productVarientId = $item['product'];
            $varientProduct = Product::allDetailsByVarientId($productVarientId);
            $varientSubTotal = $varientProduct->price * $item['stockQuantity'];
            array_push(
                $orderItems,
                [
                    'product_id' => $varientProduct['id'],
                    'variant_id' => $productVarientId,
                    'type' => 'adjustment',
                    'quantity' => $item['stockQuantity'],
                    'price' => $varientProduct->price,
                    'sub_total' => $varientSubTotal,
                    'tax_id' => $varientProduct->tax_id,
