...
class ReportController extends Controller
{
    ...
    public function inventoryReports(Request $request)
    {
        ...
        if (empty($requestType)) {
            ...
            $innventories = $inventory;
        }

        if (empty($requestType)) {
            $inventoryItems = $this->calculateInventory($innventories);

            $arrayCount = $inventoryItems['count'];
            $totalCount = count($inventory['allData']);
            $innventories[$arrayCount] = [
            ...
            $this->calculateInventory($innventories);
            return ['datarows' => $innventories];
        }

    }

    public function calculateInventory($innventories){
        $netPurchasePrice = 0;
        $netSellingPrice = 0;
        $arrayCount = 0;
        $netInventory = 0;

        foreach ($innventories as $rowData) {
            if ($rowData->type == 'customer') {
                $rowData->type = Lang::get('lang.customer');
            } else {
                $rowData->type = Lang::get('lang.internal_sales');
    ...
    public function getSalesReportFilterData()
    {
        ...
        $customers = Customer::select([DB::raw('concat(first_name," ",last_name) as text'), 'id as value'])->get();
